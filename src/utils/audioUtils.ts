/**
 * Decodes a base64 string into a Uint8Array.
 * @param base64 The base64 audio string to decode
 * @returns Uint8Array representation of the audio
 */
export function decodeBase64Audio(base64: string): Uint8Array {
    const binaryString = atob(base64);
    const len = binaryString.length;
    const bytes = new Uint8Array(len);
    for (let i = 0; i < len; i++) {
        bytes[i] = binaryString.charCodeAt(i);
    }
    return bytes;
}

/**
 * Decodes audio data into an AudioBuffer using the provided AudioContext.
 * @param data The Uint8Array audio data to process
 * @param ctx The AudioContext
 * @param sampleRate The desired sample rate for playback
 * @param numChannels Number of channels
 * @returns AudioBuffer ready for playback
 */
export async function decodeAudioData(
    data: Uint8Array,
    ctx: AudioContext,
    sampleRate: number = 24000,
    numChannels: number = 1
): Promise<AudioBuffer> {
    const dataInt16 = new Int16Array(data.buffer);
    const frameCount = dataInt16.length / numChannels;
    const buffer = ctx.createBuffer(numChannels, frameCount, sampleRate);

    for (let channel = 0; channel < numChannels; channel++) {
        const channelData = buffer.getChannelData(channel);
        for (let i = 0; i < frameCount; i++) {
            channelData[i] = dataInt16[i * numChannels + channel] / 32768.0;
        }
    }
    return buffer;
}
